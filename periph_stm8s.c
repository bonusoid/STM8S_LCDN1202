...
void i2c_init()
{
	...
	//I2C_ITR = (1<<ITBUFEN)|(1<<ITEVTEN)|(1<<ITERREN); //Turn on interrupt
...
{
	TIM1_PSCRH = 0x00; //TIM_CLK = CLK
	TIM1_PSCRL = 0x00; //TIM_CLK = CLK
	TIM1_ARRH = (timval >> 8); //TIM RELOAD
	TIM1_ARRL = (timval & 0x00FF); //TIM RELOAD
	pwm1ch1_enable();
	TIM1_CCER1 |= (0<<TIM1_CCER1_CC1P); //Output active high
	TIM1_CCMR1 = (TIM1_OCxREF_PWM_mode1<<TIM1_CCMR1_OC1M); //PWM MODE 1 for Channel 1
	pwm1_update(0x0000); //Start Value
	TIM1_BKR = (1<<TIM1_BKR_MOE); //ENABLE MAIN OUTPUT 
	TIM1_CR1 |= (1<<TIM1_CR1_CEN); //ENABLE TIM
}

void pwm2_init(unsigned int timval)
{
	TIM2_PSCR = 0x00; //TIM_CLK = CLK
	TIM2_ARRH = (timval >> 8); //TIM RELOAD
	TIM2_ARRL = (timval & 0x00FF); //TIM RELOAD
	pwm2ch1_enable();
	TIM2_CCER1 |= (0<<TIM2_CCER1_CC1P); //Output active high
	TIM2_CCMR1 = (TIM2_OCxREF_PWM_mode1<<TIM2_CCMR1_OC1M); //PWM MODE 1 for Channel 1 
	pwm2_update(0x0000); //Start Value
	TIM2_CR1 |= (1<<TIM2_CR1_CEN); //ENABLE TIM
}

void pwm1ch1_enable()
{
	TIM1_CCER1 |= (1<<TIM1_CCER1_CC1E);
}
...
{
	TIM1_CCR1L = (pwmval & 0x00FF);
	TIM1_CCR1H = (pwmval >> 8);
}

void pwm2_update(unsigned int pwmval)
{
	TIM2_CCR1L = (pwmval & 0x00FF);
	TIM2_CCR1H = (pwmval >> 8);
}
//__________ PWM FUNCTION __________//
